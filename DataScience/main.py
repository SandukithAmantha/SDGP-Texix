...
# for x in range(int(str("A")), noOfClasses):
    myPicList = os.listdir(path + "/" + str(filename))
    #print(myPicList)
    # go in to mydata folder and then A folder and in that folder every photo you found must be added to the list
    for y in myPicList:
        curImg = cv2.imread(path + "/" + str(filename) + "/" + y)
...
# your training is A.8 , testing is A.2
...
for x in range(0, noOfClasses):
    # print(len(np.where(y_train==x)[A]))
...
'''we need to preprocess all the images in x_train array'''
...
#model.save("A/model_trained.p",'wb')
